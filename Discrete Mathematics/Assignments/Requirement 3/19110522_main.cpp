...
void output_path(ofstream &FileOut, vector<int> input, int start, int end);
...
void Dijkstra(int **a, int &vertex, int &start, int &end);
...
{
	ofstream FileOut;
	FileOut.open("ket_qua_bai_3_19110522.txt", ios_base::out);
	start--;
	end--;
	if (start == end)
	{
		FileOut << "Tong chi phi di chuyen = 0" << endl;
		FileOut << "Duong di ngan nhat cua do thi la: " << start + 1 << endl;
		FileOut.close();
		system("pause");
	}
	vector<int> dist;
	...
	output_path(FileOut, previous, start, end);
	FileOut.close();
}
int main()
{
